...
get_stats_from_pred <- function(pred_file, cluster, n_fragments, n_cells, model_name, score_threshold, score_column, include_promoters) {
	message("Processing: ", cluster)
	# pre-filtering stats
	pred <- fread(pred_file) %>% mutate(distance = distanceToTSS)
	n_genes_total <- pred %>% dplyr::select(TargetGene) %>% distinct() %>% nrow()
	n_genes_not_expr <- pred %>% dplyr::filter(!!sym(score_column) == 0) %>%
		dplyr::select(TargetGene) %>% distinct() %>% nrow()

	# filter predictions
	pred <- pred %>% dplyr::filter(!!sym(score_column) >= score_threshold)
	if (!include_promoters) {
		pred <- pred %>% dplyr::filter(class != "promoter")
	}

	enh_gene <- pred %>% dplyr::select(chr, start, end, TargetGene)
	...
	num_gene_per_enh <- enh_gene %>% group_by(chr, start, end) %>%
		tally() %>% pull(n) %>% mean()
	num_enh_per_gene <- enh_gene %>% group_by(TargetGene) %>% 
		tally() %>% pull(n) %>% mean()

	dist <- pred %>% pull(distanceToTSS) %>% mean() %>% log10()

	mean_size <- enh_gene %>% dplyr::select(chr, start, end) %>% distinct() %>%
		mutate(width = end - start) %>%
		pull(width) %>% mean()

	metric_names <- c("num_unique_fragments", "num_cells", "num_enh", "num_genes_with_enh", "num_enh_gene_links",
		"mean_num_genes_per_enh", "mean_num_enh_per_gene", "mean_log10_dist_to_tss", "mean_enh_region_size",
		"num_genes_considered", "num_genes_not_expressed")
	values <- c(n_fragments, n_cells, n_enh, n_gene, n_links,
		num_gene_per_enh, num_enh_per_gene, dist, mean_size,
		n_genes_total, n_genes_not_expr)

	res <- data.frame(metric = metric_names, value = values,  cell_cluster = cluster, model_name = model_name)
	print(res)
	return(res)
}

get_distribution_within_cell_type <- function(n, key, metric, model_params, include_promoters) {
	pred_file <- key$pred_file[n]
...
plot_distribution_one_model <- function(res, other_res, model_name, metric) {
	...
	message(stats_label)
	color <- ifelse(model_name == "multiome_powerlaw_v2", "#792374", "#006479")
...
plot_distribution_both_models <- function(metric, multiome_model_params, atac_model_params, ds_include, ct_exclude, include_promoters) {
...
cluster <- sample_key$biosample
pred_file <- sample_key$pred_file
n_fragments <- sample_key$unique_fragments
n_cells <- sample_key$n_cells
out_file <- "/oak/stanford/groups/engreitz/Users/sheth/scE2G_analysis/2024_0916_global_properties/results_v1/RERUN_multiome_pred_stats.tsv"
#out_file <- "/oak/stanford/groups/engreitz/Users/sheth/scE2G_analysis/2024_0916_global_properties/results_v1/atac_pred_stats.tsv"

model_name = "multiome_powerlaw_v2"
score_threshold = 0.164
score_column = "E2G.Score.qnorm"
include_promoters = FALSE

# res <- mapply(get_stats_from_pred, pred_file, cluster, n_fragments, n_cells, model_name, score_threshold, score_column, include_promoters, SIMPLIFY = FALSE)
# res <- rbindlist(res)
#fwrite(res, out_file, sep="\t", quote=FALSE, col.names=TRUE, row.names=FALSE)

if (TRUE){
	# params
	out_dir <- "/oak/stanford/groups/engreitz/Users/sheth/scE2G_analysis/2024_0916_global_properties/results_v1/distributions"
	multiome_model_params <- c(model_name = "multiome_powerlaw_v2", score_threshold = 0.164, score_column = "E2G.Score.qnorm",
	...
	atac_model_params <- c(model_name = "scATAC_powerlaw_v2", score_threshold = 0.183, score_column = "E2G.Score.qnorm",
		sample_key_file = "/oak/stanford/groups/engreitz/Users/sheth/scE2G_analysis/2024_0916_global_properties/config/atac_sample_key.tsv")
	include_promoters <- FALSE
	...
	metrics <- c("enh_region_size")
	for (m in metrics) {
		p <- plot_distribution_both_models(m, multiome_model_params, atac_model_params, ds_include, ct_exclude, include_promoters)
