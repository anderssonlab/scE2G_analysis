...
#  make seurat object (r_genomics_v2)
make_seurat_object <- function(h5_cell_ranger_path, projected_metadata_path, out_seu, out_meta) {
	shhh({
		library(Seurat)
		library(Signac)
		library(hdf5r)
	...
	input_data <- Read10X_h5(h5_cell_ranger_path)
	rna_counts <- input_data$`Gene Expression`
	atac_counts <- input_data$Peaks

	s <- CreateSeuratObject(counts = rna_counts)
	...
	# add projected metadata
	gex_seu <- readRDS(projected_metadata_path)
	meta <- gex_seu@meta.data
	meta_sub <- meta[, c("seurat_clusters", "prediction.score.max", "predicted_day", "predicted.id")]
	s <- AddMetaData(object = s, metadata = meta_sub)
...
save_cluster_fragment_file <- function(fragments, this_bc, out_dir) {
	...
	# sort
	# chrom_include <- paste0("chr", c(1:22, "X"))
	# all_fragments <- dplyr::filter(all_fragments, chr %in% chrom_include)
	all_fragments <- all_fragments[order(all_fragments[[1]], as.numeric(all_fragments[[2]]))]
	num_frag <- nrow(all_fragments)
	message("# fragments: ", num_frag)
...
filter_data_all_clusters <- function(seu_path, fragment_path, cluster_barcode_path, out_dir) {
	shhh({
		library(Seurat)
		library(Signac)
	})
	...
	cluster_bcs <- fread(cluster_barcode_path) # cell, seurat_clusters
	...
	for (c in seq_along(clusters)) {
		...
		cluster_out <- file.path(out_dir, paste0("cluster_", this_cluster)); dir.create(cluster_out, showWarnings = FALSE)

		this_bc <- dplyr::filter(cluster_bcs, seurat_clusters == this_cluster) %>% pull(cell)
		message("# barcodes in metadata: ", length(this_bc))

		seu_sub <- subset(seu, subset = seurat_clusters == this_cluster)
		save_cluster_fragment_file(fragments, this_bc, cluster_out)
		save_cluster_rna_matrix(seu_sub, this_bc, cluster_out)
	}
}
...
# for qc
out_dir <- "/oak/stanford/groups/engreitz/Users/sheth/scE2G_analysis/2025_0127_validation_cell_lines/WTC11_HPC_processing_lenient"
# for saving per-cluster files
final_out_dir <-  "/oak/stanford/groups/engreitz/Users/sheth/scE2G_analysis/2025_0127_validation_cell_lines/scE2G_input/WTC11_HPC_lenient"; dir.create(final_out_dir)


seu_path <- "/oak/stanford/groups/engreitz/Users/sheth/scE2G_analysis/2025_0127_validation_cell_lines/data/seu_WTC11_HPC_with_projected_metadata.rds"
meta_path <-  "/oak/stanford/groups/engreitz/Users/sheth/scE2G_analysis/2025_0127_validation_cell_lines/data/seu_WTC11_HPC_metadata.tsv.gz"
...
#make_seurat_object(h5_cell_ranger_path, projected_metadata_path, seu_path, meta_path)  # used r_genomcis_v2
...
filter_data_all_clusters(seu_path, fragment_path, cluster_barcodes_out, final_out_dir)